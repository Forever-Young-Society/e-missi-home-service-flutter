...
class ServiceAddController extends BaseController {
  ...
  initializeTextEditingController() {
    ...
    identityTextController =
        TextEditingController(text: bookingDetailDataModel?.icNumber?.toString() ?? "");
    emailTextController =
        TextEditingController(text: bookingDetailDataModel?.email?.toString() ?? "");
    genderTextController = TextEditingController(
        text: bookingDetailDataModel?.gender == 0 ? "Male" : "Female");
    ...
    serviceReportTextController = TextEditingController(text: bookingDetailDataModel?.description ?? "");
  ...
  CompleteBookingResponseModel? completeBookingResponseModel;

  hitAddServiceReportApi() {
    Map<String, dynamic> requestModel =
        AuthRequestModel.addServiceReportRequestModel(
      description: serviceReportTextController?.text.trim().toString() ?? "",
    );
    _repository
        .bookingReportPostApiCall(
            dataBody: requestModel, bookingID: bookingDetailDataModel?.bookingId)
        .then((value) async {
      if (value != null) {
        completeBookingResponseModel=value;
        Get.back(result: "data");
        flashBar(message: completeBookingResponseModel?.message?.toString()??"");
        update();
      }
